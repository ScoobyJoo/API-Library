...
def test_create_customer_missing_fields(client):
    respond = client.post("/api/customers", json={"first_name": "Ada"})
...
def test_create_customer_duplicate_email(client):
    ...
    respond = create_customer(client, first_name="Charles", last_name="Babbage")
...
def test_create_customer_bad_membership_date(client):
    respond = create_customer(client, membership_date="not-a-date")
    assert respond.status_code == 400

# Test creating a customer with an explicit membership date
def test_create_customer_explicit_membership_date(client):
    respond = create_customer(client, membership_date="2020-01-15")
    assert respond.status_code == 201
    assert respond.get_json()["membership_date"] == "2020-01-15"

# Create a customer and fetch it by ID
def test_get_customer_by_id(client):
    created = create_customer(client).get_json()
    respond = client.get(f"/api/customers/{created['id']}")
    assert respond.status_code == 200
    assert respond.get_json()["last_name"] == "Lovelace"
...
def test_update_customer(client):
    created = create_customer(client).get_json()
    respond = client.patch(f"/api/customers/{created['id']}", json={"phone": "555-9999"})
    assert respond.status_code == 200
    assert respond.get_json()["phone"] == "555-9999"
...
def test_update_customer_duplicate_email(client):
    ...
    respond = client.patch(f"/api/customers/{other['id']}", json={"email": "ada@example.com"})
    assert respond.status_code == 409

# Test that updating a customer that does not exist returns a 404
def test_update_customer_not_found(client):
    respond = client.patch("/api/customers/999", json={"phone": "555-0000"})
    assert respond.status_code == 404

# Delete a customer
def test_delete_customer(client):
    created = create_customer(client).get_json()
    respond = client.delete(f"/api/customers/{created['id']}")
